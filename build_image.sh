...
DEBOOTSTRAP_CMD="debootstrap --verbose --no-check-gpg --foreign --arch=armhf buster ${CHROOT_DIR} http://archive.raspbian.org/raspbian"
...
chroot "${CHROOT_DIR}" /bin/bash -c 'DEBIAN_FRONTEND=noninteractive; DEBCONF_NONINTERACTIVE_SEEN=true; /debootstrap/debootstrap --verbose --second-stage'

echo "Cleaning apt cache in ${CHROOT_DIR}"
chroot "${CHROOT_DIR}" apt-get clean

cp ./configure-server.sh "${CHROOT_DIR}"/usr/bin/
chmod +x "${CHROOT_DIR}"/usr/bin/configure-server.sh
